...
1. **Clone the Repository**:
    ```bash
    git clone https://github.com/DebakantPradhan/upiFraudDetection.git
...
4. **Set Up the Database**:
   Ensure that the `upi_fraud_dataset.csv` file is placed in the `src/` directory. This dataset will be first cleaned and the new `train_dataset.csv` in the new folder `./dataset` used for training and evaluating the machine learning models.
